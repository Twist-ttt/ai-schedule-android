...
- **提示词原文**：「请你确保所有非应用脚本都删干净了」
- **解决的问题**：打包交付前，仓库只保留应用本身与交付物，清除辅助/临时脚本。
- **AI 生成结果与我的修改**：排查全项目脚本类文件（`*.py/sh/ps1/cmd/bat`），确认非应用脚本仅 `md2docx.py`（`gradlew`/`gradlew.bat` 为 Gradle wrapper 属应用构建，保留）；`git rm` 删除之，其生成的 docx 交付物（`项目设计文档.docx`、`AI提示词使用记录.docx`）保留。
